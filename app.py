...
.hero-sub {
    text-align: center; color: #7fb3d3; font-size: 1rem;
    font-weight: 300; letter-spacing: 0.1em; margin-bottom: 2rem;
}
.fruit-card {
...
.badge-ripe {
    ...
    display: block; margin: 1rem 0; font-family: 'Playfair Display', serif;
}
.badge-fresh {
...
[data-testid="stMetricLabel"] { color: #e8f4f8 !important; }
[data-testid="stSlider"] label { color: #a8d8ea !important; font-weight: 500; }
.section-title {
    color: #a8e6cf; font-family: 'Playfair Display', serif;
    font-size: 1.2rem;
...
.prob-bar-bg {
    ...
    border-radius: 8px; height: 12px; margin: 4px 0 12px;
}
.prob-bar-fill-green {
...
.prob-label {
    color: #cde8f5; font-size: 0.85rem;
...
with left:
    st.markdown('<div class="section-title">📐 Fruit Measurements</div>',
                unsafe_allow_html=True)
    size          = st.slider("📏 Size (cm)",          3.0, 10.0,  6.5, step=0.1)
    weight        = st.slider("⚖️ Weight (g)",         50.0,300.0,150.0, step=1.0)
    colour_score  = st.slider("🎨 Colour Score",        1.0, 10.0,  5.0, step=0.1)
    firmness      = st.slider("💪 Firmness",             1.0, 10.0,  5.0, step=0.1)
    sugar_content = st.slider("🍬 Sugar Content",        1.0, 10.0,  5.0, step=0.1)
    blemish_score = st.slider("🔍 Blemish Score",        0.0, 10.0,  2.0, step=0.1)
    predict_btn   = st.button("🔬 Analyse Fruit Quality",
    ...
    if not predict_btn:
        ...
        <div class="fruit-card">
            ...
            <div style="color:#cde8f5; font-size:0.9rem; line-height:1.7;">
                Trained on <strong>500 synthetic fruit samples</strong>
        ...
        input_df    = pd.DataFrame([[size, weight, colour_score,
        ...
        if quality == 'Ripe':
            st.markdown('<span class="badge-ripe">🟢 RIPE — Perfect Quality!</span>',
                        unsafe_allow_html=True)
        elif quality == 'Fresh':
            st.markdown('<span class="badge-fresh">🔵 FRESH — Needs More Time</span>',
                        unsafe_allow_html=True)
        else:
            st.markdown('<span class="badge-rotten">🔴 ROTTEN — Do Not Consume</span>',
                        unsafe_allow_html=True)
        ...
        for label, (val, mn, mx) in scores.items():
            ...
            if "Blemish" in label:
                color = "#e74c3c" if pct > 60 else ("#f39c12" if pct > 30 else "#56ab2f")
            else:
                color = "#56ab2f" if pct > 60 else ("#f39c12" if pct > 30 else "#e74c3c")
            st.markdown(f"""
            <div style="display:flex;justify-content:space-between;
                        color:#cde8f5;font-size:0.88rem;margin-bottom:2px;">
                <span>{label}</span>
        ...
        st.markdown('<div class="section-title">💡 Recommendation</div>',
                    unsafe_allow_html=True)
        if quality == 'Ripe':
            st.success("🌟 Perfect fruit! Best consumed within 1-2 days.")
        elif quality == 'Fresh':
            st.info("⏳ Needs 2-3 more days to ripen. Store at room temperature.")
        else:
            tips = []
            if firmness      < 3:   tips.append("💪 Very low firmness — fruit has gone soft.")
            if blemish_score > 7.5: tips.append("🔍 Very high blemish score — too much damage.")
            if colour_score  < 2.5: tips.append("🎨 Poor colour score — signs of decay.")
            for t in tips:
                st.warning(t)

        st.divider()
        st.markdown('<div class="section-title">📊 Which Properties Matter Most?</div>',
                    unsafe_allow_html=True)
        imp_df = pd.DataFrame({
            'Property'  : feature_names,
            'Importance': model.feature_importances_
        }).sort_values('Importance', ascending=False)
        st.bar_chart(imp_df.set_index('Property'), color="#56ab2f")
